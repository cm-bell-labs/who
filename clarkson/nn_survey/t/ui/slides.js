...
function currentSlide() {
	...
	var foo = document.createTextNode(snum + "\/" + (smax-1));
	cs.replaceChild(foo, cs.firstChild);
...
document.getElementById(cid).setAttribute("style", "display: none");
ne.setAttribute("style", "display: block; visibility: visible");
/*
...
function notOperaFix() {
	...
	obj = document.getElementById('slidePrint');
	obj.setAttribute('media','print');
	if (isGe) {
		obj.setAttribute('href','null');   // Gecko fix
		obj.setAttribute('href',slideCSS); // Gecko fix
	}
...
if (!isGe) {
	alert("Sorry, this looks better in Mozilla");
	} else {
		...
		if (!isOp) {		
			notOperaFix();
			slideJump();
	document.getElementById('slide0').setAttribute("style", "display: block; visibility: visible")
