...
function graded_t(x) {return x*0.3 + 5; }  //x*0.74 + 5;} //{return x*0.584+5;}
...
function graded_draw_initial_enet()
{
	...
	for (var i=0; i<graded_triang_bound_enet; i++) {
		gc_ctx.beginPath();
		gc_ctx.arc(graded_t(graded_triang_coords[2*i]), graded_t(graded_triang_coords[2*i+1]), 3, 0, 2*Math.PI, true);
		gc_ctx.fill();
	}
}
...
//		71, 10, "canvas_enet_mp", "", "first_enet_point_mp", 0.2, ok_for_mp);
	...
	cur_enet = ""; //enet_graded;
//	enet_biotope_rad = enet_biotope.rad;
//	enet_hyperbolic_rad = enet_hyperbolic.rad;
	
	var ets = new Array("_graded"); //, "_biotope", "_hyperbolic", "_graded");
...
function draw_title_canvas()
{
	...
	gc_ctx.strokeStyle = "red";
 	for (i=0; i<points_x.length; i++) {
		...
		gc_ctx.beginPath();
		gc_ctx.arc(points_x[i], points_y[i], 3, 0, 2*Math.PI, true);
		gc_ctx.fill();
	}
}
...
function one_step_graded()
{
	...
	gc_ctx.strokeStyle = gc_ctx.fillStyle = "blue";
	gc_ctx.beginPath();
	gc_ctx.arc(graded_t(graded_triang_coords[2*graded_top]), graded_t(graded_triang_coords[2*graded_top+1]), 3, 0, 2*Math.PI, true);
	gc_ctx.fill();
...
{
	if (!one_step())
		anim_running = setTimeout("animate()",5);
	else
		need_reset = 1;
}
